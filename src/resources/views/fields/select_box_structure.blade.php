...
@php
    ...
    if (!function_exists('echoSelect2NestedEntry')) {
        function echoSelect2NestedEntry($entry, $field, $current_value, $depth = 0) {
            ...
            echo str_repeat("-", (int)$depth >= 1 ? (int)$depth : 0).' '.$entry->{$field['attribute']};
            echo "</option>";
        }
    }

    if (!function_exists('echoSelect2NestedChildren')) {
        function echoSelect2NestedChildren($entity, $field, $current_value, $depth = 0)
        {
            ++$depth;
         foreach ($entity->children()->get() as $entry)
            {
                echoSelect2NestedEntry($entry, $field, $current_value, $depth);
                echoSelect2NestedChildren($entry, $field, $current_value, $depth);
            }
        }
    }
@endphp
...
>
    ...
    @if (isset($field['model']))
        @php
            $obj = new $field['model'];
            $first_level_items = $obj->where('parent_id', '0')->orWhere('parent_id', null)->orderBy('order', 'ASC')->get();
        @endphp
